...
namespace ClearDB
{
	public class CopyData
	{
		...
		public static void WriteCopyObj(DateTime dateStart, DateTime dateEnd, List<int> objects, string DBName, string fromDBName = "P3000") {
			Logger.Info(String.Format("{0} - {1}", dateStart, dateEnd));

			string sel = String.Format("SELECT * FROM DATA where object in ({2}) and data_date>='{0}' and data_date<='{1}'",
				dateStart.ToString(DBClass.DateFormat), dateEnd.ToString(DBClass.DateFormat), String.Join(",", objects));
